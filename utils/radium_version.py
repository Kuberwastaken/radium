...
"""
Script to generate Radium version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "RADIUM" in file.read():
            raise ValueError("file already contains radium versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "RADIUM_MAJOR": tree / "version.txt",
        "RADIUM_MINOR": tree / "chromium_version.txt",
        "RADIUM_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["RADIUM_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "RADIUM_MINOR" else -CHROME_VERSION_BASE
